...
//
// Категория: string segmentation DP.
// Состояние: dp[i] = можно ли разбить префикс s[0..i-1].
// Переход: dp[i] = OR по всем j <= i: dp[j] AND s[j..i-1] in wordDict.
// База: dp[0] = true (пустая строка разбивается).
